...
import { Store, ShieldCheck, Truck, LineChart, Loader2, Sparkles, ShoppingBag, Info, Eye, Star } from 'lucide-react';
...
export function ProductSideInvoice({ product, seller, sellersCount = 1 }: ProductSideInvoiceProps) {
  ...
  const discountPercentage = seller.hasDiscount && seller.retailPriceRaw && seller.finalPriceRaw
    ? Math.round(((seller.retailPriceRaw - seller.finalPriceRaw) / seller.retailPriceRaw) * 100)
    : (seller.discountPercentage || 0);

  return (
    <Card className="w-full border rounded-xl overflow-hidden bg-background shadow-sm text-right flex flex-col select-none">
      
      {seller.hasDiscount && discountPercentage > 0 && (
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3 w-full border-b pb-3.5 text-xs text-muted-foreground font-iran-sans">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-foreground">ارسال از {seller.shop.address.split('،')[0]} | {seller.dayOfDeliveryLabel}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 py-3 border-b border-dashed w-full text-center shrink-0">
          <div className="flex flex-col items-center justify-center gap-1">
            ...
            <span className="text-[10px] font-bold text-foreground font-iran-sans leading-none mt-1">{product.salesCount || '۰'} خرید</span>
          </div>
          <div className="flex flex-col items-center justify-center gap-1 border-x dark:border-zinc-800">
            ...
            <span className="text-[10px] font-bold text-foreground font-iran-sans leading-none mt-1">{product.views || '۰'} بازدید</span>
          </div>
          <div className="flex flex-col items-center justify-center gap-1">
            <Star className="h-4.5 w-4.5 fill-yellow-400 text-yellow-400 shrink-0" />
            <span className="text-[10px] font-bold text-foreground font-iran-sans leading-none mt-1">امتیاز {product.averageRate || '۵'}</span>
          </div>
        </div>
        ...
        <div className="w-full flex items-center justify-between pt-1">
          <div className="flex flex-col text-right">
            ...
            <span className="text-base md:text-lg font-black text-foreground font-iran-sans leading-none">{seller.finalPrice}</span>
          </div>
