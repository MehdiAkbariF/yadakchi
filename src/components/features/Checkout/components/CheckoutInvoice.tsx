  ...
  return (
    <Card className="w-full lg:w-[360px] shrink-0 border rounded-xl p-5 bg-background shadow-sm h-fit text-right">
      ...
      <div className="space-y-4 border-b pb-4 mb-4 text-right">
        <div className="flex items-center justify-between text-xs font-medium font-iran-sans text-muted-foreground">
          <span>قیمت محصولات</span>
          <span>{formatNumber(basket.totalOriginalPrice / 10)} تومان</span>
        </div>

        {basket.totalDiscountValue > 0 && (
          <div className="flex items-center justify-between text-xs font-bold font-iran-sans text-destructive">
            <span>تخفیف‌ها</span>
      ...
      <div className="flex items-center justify-between mb-6 text-right">
        <span className="text-xs font-bold font-iran-sans text-muted-foreground">مبلغ نهایی قابل پرداخت</span>
        <span className="text-lg font-black font-iran-sans text-foreground">{formatNumber(finalPayablePrice / 10)} تومان</span>
      </div>
